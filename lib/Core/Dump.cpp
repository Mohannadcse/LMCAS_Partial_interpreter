...
/*
 * MOH:: this method iterates functions and prints out their corresponding
 * visited BB This method is invoked with --dump-bbs
 */
void writeVisitedBBs(std::ofstream &dumpFile, KModule *kmodule) {
  ...
  for (auto curF = module->getFunctionList().begin(),
       ...
       curF != endF; ++curF) {
    int i = 0;
    for (auto curB = curF->begin(), endB = curF->end(); curB != endB; ++curB) {
      llvm::BasicBlock *bb = &(*curB);
      if (kmodule->visitedBBs.find(bb) != kmodule->visitedBBs.end()) {
        std::string name = curF->getName().str();
        if (name.compare("__klee_posix_wrapped_main") ==
            0) // name.compare("__user_main") == 0 ||
          name = "main";
        dumpFile << name << " " << i << "\n";
      }
      ++i;
...
void Dump::dumpGlobals() {
  ...
  for (auto const &kv : executor.globalAddresses) {
    ...
    if (auto gvar = dyn_cast<llvm::GlobalVariable>(
            kv.first)) {
      if (gvar->getType()->getElementType()->isPointerTy()) {
        addAddress(ty, address);
      }
      //for handling global i8* 
...
void Dump::dumpStack() {
  ...
  for (auto it = state.stack.crbegin(); it != state.stack.crend(); ++it) {
    ...
    // this loop collects the type of each instruction, and excludes any intr of
    // void type
    ...
    for (unsigned reg = 0; reg < it->kf->numInstructions; ++reg) {
      //			logger << "kf= "<< kf << " ---numArgs: "<<
      // kf->numArgs << " ---numRegisters: " <<kf->numRegisters << "
      //---numInstructions: " << kf->numInstructions<<"\n";
      auto &&value = it->locals[reg + kf->numArgs].value;
      if (value.get()) {
        if (ConstantExpr *constantValue = llvm::dyn_cast<ConstantExpr>(value)) {
          loggerInst << "\t" << *regtypes[reg] << " --- " << reg << " --- "
                     << constantValue->getZExtValue() << "\n";

          stack.back().locals.push_back(
              std::make_tuple(regtypes[reg], reg, constantValue->getZExtValue(),
                              it->kf->instructions[reg]->inst));

          if (llvm::PointerType *pt =
                  dyn_cast<llvm::PointerType>(regtypes[reg])) {

            /*
             * I added these 2 conditions to make sure the type of the allocated
             * variable isn't an array Allocated array leads to segmentation
             * fault. I had this case in the toy wc
             */
    ...
    for (unsigned index = 0; index < kf->numInstructions; index++) {
      ...
      if (value.get())
        if (ConstantExpr *constantValue = llvm::dyn_cast<ConstantExpr>(value)) {
          instKLEE << "K: " << index << " --- " << *I << " --- "
                   << constantValue->getZExtValue() << "\n";
          addrToStackVars.emplace(constantValue->getZExtValue(),
                                  std::make_tuple(I, index));
        }
      kleeInstrs.push_back(I);
      if (auto gep = dyn_cast<llvm::GetElementPtrInst>(I))
        for (auto ii = gep_type_begin(gep); ii != gep_type_end(gep); ii++)
          if (llvm::StructType *st = dyn_cast<llvm::StructType>(*ii)) {
            const llvm::StructLayout *sl =
                executor.kmodule->targetData->getStructLayout(st);
            const llvm::ConstantInt *ci =
                cast<llvm::ConstantInt>(ii.getOperand());
            uint64_t addend =
                sl->getElementOffset((unsigned)ci->getZExtValue());
            instKLEE << "\tsl: " /*<< *st*/ << "---ci: " << ci->getZExtValue()
                     << "---addend: " << addend << "\n";
          }
    }
  }
...
/*
 * This method is invoked when the argument --dump-file without --dump-bbs
 * This method iterates the memory where the results of symbolic execution is
 * stored, and gets the values of certain datatypes
 */
void Dump::dumpMemoryRegions() {
  std::string log;
  llvm::raw_string_ostream loggerInst(log);
  // this hack is required to handle struct elements, when I read the element
  // <os->read(i, width*8)> in the else scope
  ...
  while (hasUnvisitedAddress()) {
    ...
    if (llvm::StructType *st = dyn_cast<llvm::StructType>(ty)) {
      ...
      std::vector<std::tuple<uint64_t, int, llvm::StringRef>>
          tmpStrElemVecNested;
      std::vector<std::tuple<uint64_t, int>> tmpStrElemVec;
      for (unsigned i = 0; i < st->getNumElements(); ++i) {
        auto width = Context::get().getPointerWidth();
        auto addend = ConstantExpr::create(sl->getElementOffset(i), width);
        ...
        if (isa<llvm::StructType>(ty->getContainedType(i))) {
          ...
          tmpStrElemVecNested.push_back(std::make_tuple(
              thisaddress.get()->getZExtValue(), i, nestedStrctName));
        } else {
          tmpStrElemVec.push_back(
              std::make_tuple(thisaddress.get()->getZExtValue(), i));
        }

        loggerInst << "thisAddress:: " << thisaddress.get()->getZExtValue()
                   << "---Kind: " << thisaddress.get()->getKind()
                   << "---stIdx=" << i << "---addend: " << addend
                   << "---Nested: "
                   << "\n";
      }
      if (!tmpStrElemVecNested.empty()) {
        mainStctElemStrct.emplace(
            std::make_tuple(st->getName(), addr.get()->getZExtValue()),
            tmpStrElemVecNested);
      }
      ...
      if (state.addressSpace.resolveOne(addr, op)) {
        ...
        // reads a concrete string from memory inspired by
        ...
        // inspired by the assertion Assertion `width == NumBytes * 8 &&
        // "Invalid width for read size!" 				int i = 0;
        // auto it =
        // strctAddrToOffset.find(addr.get()->getZExtValue());
        // if (it !=
        // strctAddrToOffset.end()) { 					i =
        // it->second;
        //				}
        ref<Expr> value = os->read(offset, width * 8);
        if (!value.isNull()) {
          loggerInst << "\tvalue " << value << "\n";
          //					loggerInst << "\tValue_offset: " <<
          //os->read(i, width * 8) <<"\n" ;
        }
        if (!value.isNull())
          if (ConstantExpr *constantValue =
                  llvm::dyn_cast<ConstantExpr>(value)) {
            if (constantValue->getWidth() <= 64) {
              ...
              if (memory.count(addrValue) == 0) {
                memory.insert(
                    {addrValue, {{ty, constantValue->getZExtValue()}}});
              ...
              if (llvm::PointerType *pt = dyn_cast<llvm::PointerType>(ty)) {
                if (llvm::StructType *st =
                        dyn_cast<llvm::StructType>(pt->getElementType())) {
                  loggerInst << "\tStr:: " << *st << "\n";
                  const llvm::StructLayout *sl =
                      executor.kmodule->targetData->getStructLayout(st);

                  std::vector<
                      std::tuple<uint64_t, int, llvm::StringRef, uint64_t>>
                      tmpPtrMainStrctElemStrct;
                  std::vector<
                      std::tuple<uint64_t, int, llvm::StringRef, uint64_t>>
                      tmpPtrMainStrctPtrElemStrct;
                  std::vector<std::tuple<uint64_t, int>> tmpStrElemVecPTR;
                  unsigned numElements = st->getNumElements();
                  for (unsigned i = 0; i < numElements; ++i) {
                    auto width = Context::get().getPointerWidth();
                    auto addend =
                        ConstantExpr::create(sl->getElementOffset(i), width);
                    ref<ConstantExpr> thisaddress = addr->Add(addend);
                    addAddress(st->getElementType(i), thisaddress);
                    loggerInst
                        << "PT_Address:: " << thisaddress.get()->getZExtValue()
                        << "---stIdx=" << i << "---addend: " << addend
                        << "---consta: " << constantValue->getZExtValue()
                        << "---sum: "
                        << constantValue->getZExtValue() +
                               sl->getElementOffset(i)
                        << "\n";
                    if (st->getElementType(i)->isStructTy()) {
                      auto nestedStrctName =
                          st->getElementType(i)->getStructName();
                      tmpPtrMainStrctElemStrct.push_back(std::make_tuple(
                          thisaddress.get()->getZExtValue(), i, nestedStrctName,
                          constantValue->getZExtValue()));
                    } else if (llvm::PointerType *pstST =
                                   dyn_cast<llvm::PointerType>(
                                       st->getTypeAtIndex(i))) {
                      if (llvm::StructType *nestedStrctName =
                              dyn_cast<llvm::StructType>(
                                  pstST->getElementType())) {
                        tmpPtrMainStrctPtrElemStrct.push_back(
                            std::make_tuple(thisaddress.get()->getZExtValue(),
                                            i, nestedStrctName->getName(),
                                            constantValue->getZExtValue()));
                      }
                    } else {
                      tmpStrElemVecPTR.push_back(
                          std::make_tuple(constantValue->getZExtValue() +
                                              sl->getElementOffset(i),
                                          i));
                    }
                  }
                  if (!tmpPtrMainStrctElemStrct.empty()) {
                    ptrMainStrctElemStrct.emplace(
                        std::make_tuple(st->getName(),
                                        addr.get()->getZExtValue()),
                        tmpPtrMainStrctElemStrct);
                  }
                  if (!tmpPtrMainStrctPtrElemStrct.empty()) {
                    ptrMainStrctPtrElemStrct.emplace(
                        std::make_tuple(st->getName(),
                                        addr.get()->getZExtValue()),
                        tmpPtrMainStrctPtrElemStrct);
                  }
                  if (!tmpStrElemVecPTR.empty()) {
                    structVarsPTR.emplace(
                        std::make_tuple(st->getName(),
                                        addr.get()->getZExtValue()),
                        tmpStrElemVecPTR);
                  }
                ...
                if (pt->getElementType()->isSized()) {
                  addAddress(pt->getElementType(), constantValue);
                  if (isa<llvm::IntegerType>(pt->getElementType())) {
                    primitivePtrToVal.emplace(addrValue,
                                              constantValue->getZExtValue());
                  }
                  loggerInst
                      << "\tpt:: " << *pt
                      << "---elemTyp: " << *pt->getElementType()
                      << "---constantValue: " << constantValue->getZExtValue()
                      << "\n";
                }
              }
            }
  ...
  for (auto elem : mainStctElemStrct)
    for (auto nestdElems : elem.second) {
      ...
      if (itt != addrToStackVars.end())
        loggerInst << std::get<0>(elem.first) << "::" << std::get<1>(elem.first)
                   << "---" << std::get<0>(nestdElems) << "---"
                   << std::get<1>(nestdElems) << "---"
                   << std::get<2>(nestdElems) << "---"
  ...
  for (auto f : structVars) {
    loggerInst << "\t#Records: " << std::get<0>(f.first)
               << "::" << std::get<1>(f.first) << "---" << f.second.size()
               << "\n";
    if (f.second.empty()) {
      loggerInst << "REMOVE this record\n";
      auto it = structVars.find(
          std::make_tuple(std::get<0>(f.first), std::get<1>(f.first)));
      structVars.erase(it);
    }
    // int i = 0;
    ...
    // 		loggerInst << "\t\t\t"<< std::get<0>(f.first) << "::" <<
    // std::get<1>(f.first) << "---" << std::get<0>(*v) <<  "---" <<
    // std::get<1>(*v) << "---" << *val << "\n"; 	} else { 		loggerInst
    // <<
    // "\tREMOVE this element\n"; 		f.second.erase(v);
    // 	}
  ...
  loggerInst << "\nPrint ptrMainStrctElemStrct: "
             << ptrMainStrctElemStrct.size() << "\n";
  for (auto elem : ptrMainStrctElemStrct)
    for (auto nestdElems : elem.second) {
      ...
      if (itt != addrToStackVars.end())
        loggerInst << std::get<0>(elem.first) << "::" << std::get<1>(elem.first)
                   << "---" << std::get<0>(nestdElems)
                   << "---1: " << std::get<1>(nestdElems)
                   << "---2: " << std::get<2>(nestdElems)
                   << "---3: " << std::get<3>(nestdElems) << "---"
                   << *std::get<0>(itt->second) << "\n";
    }

  loggerInst << "\nPrint ptrMainStrctPtrElemStrct: "
             << ptrMainStrctPtrElemStrct.size() << "\n";
  ...
  for (auto f : structVarsPTR)
    for (auto v : f.second) {
      ...
      // std::get<1>(v) << "---" << "\n";
      if (auto val = getAddrConcValue(std::get<0>(v), 0)) {
        loggerInst << std::get<0>(f.first) << "::" << std::get<1>(f.first)
  ...
  for (auto f : addrToStackVars) {
    loggerInst << f.first << "---" << *std::get<0>(f.second) << "---"
               << std::get<1>(f.second) << "\n";
  }
...
void Dump::handlePtrToStrct() {
  ...
  for (auto elem : ptrMainStrctElemStrct)
    for (auto nestdElems : elem.second) {
      ...
      auto nstdSt = mainStctElemStrct.find(
          std::make_tuple(std::get<0>(elem.first), std::get<3>(nestdElems)));
      if (nstdSt != mainStctElemStrct.end()) {
        ...
        for (auto nestdElemsPTR : nstdSt->second) {
          auto itt = addrToStackVars.find(std::get<1>(elem.first));
          auto itST = structVars.find(std::make_tuple(
              std::get<2>(nestdElemsPTR), std::get<0>(nestdElemsPTR)));
          if (itST != structVars.end() && itt != addrToStackVars.end()) {
            for (auto e : itST->second)
              if (auto val = getAddrConcValue(std::get<0>(e), 1)) {
                writeNestedStrctVars(std::get<0>(elem.first),
                                     std::get<2>(nestdElemsPTR),
                                     std::get<1>(nestdElemsPTR), std::get<1>(e),
                                     *val, 1, std::get<1>(itt->second));
              }
          }
        }
      }
...
void Dump::handleStructsCalledByAddress() {
  ...
  for (auto elem : mainStctElemStrct)
    for (auto nestdElems : elem.second) {
      auto itt = addrToStackVars.find(std::get<1>(elem.first));
      auto itST = structVars.find(
          std::make_tuple(std::get<2>(nestdElems), std::get<0>(nestdElems)));
      if (itST != structVars.end() && itt != addrToStackVars.end()) {
        for (auto e : itST->second)
          if (auto val = getAddrConcValue(std::get<0>(e), 1)) {
            ...
            writeNestedStrctVars(std::get<0>(elem.first),
                                 std::get<2>(nestdElems),
                                 std::get<1>(nestdElems), std::get<1>(e), *val,
                                 0, std::get<1>(itt->second));
          }
    ...
    for (auto v : f.second) {
      ...
      if (itt != addrToStackVars.end())
        if (auto val = getAddrConcValue(std::get<0>(v), 1)) {
          writeStrctVars(std::get<0>(f.first), std::get<1>(v), *val,
    ...
    for (auto vPTR : f.second) {
      ...
      if (itt != addrToStackVars.end())
        if (auto val = getAddrConcValue(std::get<0>(vPTR), 0)) {
          //				llvm::outs() << "HERE:: " << std::get<0>(f.first)
          //<<
          //"::" << std::get<1>(f.first) <<  "---" << std::get<0>(vPTR) << "---"
          //<< std::get<1>(vPTR) << "---" << *val << "\n";
          writePtrToStructStr(std::get<0>(f.first), std::get<1>(vPTR), *val,
                              std::get<1>(itt->second));
        }
    }
}

int getActualIdx(std::vector<std::tuple<llvm::Type *, unsigned, uint64_t,
                                        llvm::Instruction *>>
                     locals,
                 uint64_t actualAddr) {
  int idx = -1;
 ...
 */
void Dump::handleStructPrimitiveElem(llvm::GetElementPtrInst *origGEP,
                                     uint64_t val) {
  ...
  //	logger << "\t" << *origGEP <<"\n";
  auto opr0Type =
      dyn_cast<llvm::PointerType>(origGEP->getOperand(0)->getType());

  if (opr0Type) {
    if (auto pt = dyn_cast<llvm::StructType>(opr0Type->getElementType()))
      if (auto op2 = dyn_cast<llvm::ConstantInt>(origGEP->getOperand(2))) {
        int allocIdx = getAllocInstrIdxUsedByStrct(origGEP);
        writeStrctVars(pt->getStructName(), op2->getValue().getZExtValue(), val,
                       allocIdx);
        logger << "\tstructVars\n";
      }
  }
...
int Dump::getAllocInstrIdxUsedByStrct(llvm::Instruction *I) {
  ...
  if (auto ai = dyn_cast<llvm::AllocaInst>(I->getOperand(0))) {
    auto alloc =
        find(kleeInstrs.begin(), kleeInstrs.end(), cast<llvm::Instruction>(ai));
...
void Dump::writeStrctVars(llvm::StringRef strctName, uint64_t elemIdx,
                          uint64_t val, int allocIdx) {
  ...
  structVars << strctName << " " << elemIdx << " " << val << " " << allocIdx
             << "\n";
...
/*
 * Format:
 * mainNameStrct-idxInMainStrct-structNameElem-idxStructElem-value--isPtrElem(1
 * if the struct element is ptr)-allocIdx
 */
void Dump::writeNestedStrctVars(llvm::StringRef mainStruct,
                                llvm::StringRef elementStruct,
                                uint64_t idxInMainStrct, uint64_t idxStructElem,
                                uint64_t val, int isPtrElem, int allocIdx) {
  ...
  nestedStrctVars << mainStruct << " " << idxInMainStrct << " " << elementStruct
                  << " " << idxStructElem << " " << val << " " << isPtrElem
                  << " " << allocIdx << "\n";
      ...
      if (auto op2 = dyn_cast<llvm::ConstantInt>(origGEP->getOperand(2))) {
        if (auto opr0TypeGepOprnd = dyn_cast<llvm::PointerType>(
          ...
          if (auto ptOpr = dyn_cast<llvm::StructType>(
            ...
            if (auto op2OprGep =
                    dyn_cast<llvm::ConstantInt>(gepOprnd->getOperand(2))) {
              int idx = getAllocInstrIdxUsedByStrct(gepOprnd);
              if (idx != -1) {
                writeNestedStrctVars(
                    ptOpr->getStructName(), pt->getStructName(),
                    op2OprGep->getValue().getZExtValue(),
                    op2->getValue().getZExtValue(), val, 0, idx);
              }
            }
        }
        logger << "\tstructVars\n";
  ...
  if (auto opr0Type =
    ...
    if (auto pt = dyn_cast<llvm::StructType>(opr0Type->getElementType()))
      if (auto op2 = dyn_cast<llvm::ConstantInt>(origGEP->getOperand(2))) {
        ...
        // 1- GEP this means this struct is a pointer to struct but an element
        // of another struct 2- Alloc this means this struct is a pointer to
        // struct
        if (auto ld = dyn_cast<llvm::LoadInst>(origGEP->getOperand(0))) {
          if (auto *gep =
                  dyn_cast<llvm::GetElementPtrInst>(ld->getPointerOperand())) {
            if (auto gepOpr0Type = dyn_cast<llvm::PointerType>(
                    gep->getOperand(0)->getType())) {
              if (auto ptGepOpr =
                      dyn_cast<llvm::StructType>(gepOpr0Type->getElementType()))
                if (auto gepOp2 =
                        dyn_cast<llvm::ConstantInt>(gep->getOperand(2))) {
                  int allocIdx = getAllocInstrIdxUsedByStrct(gep);
                  if (allocIdx != -1)
                    writeNestedStrctVars(
                        ptGepOpr->getStructName(), pt->getStructName(),
                        gepOp2->getValue().getZExtValue(),
                        op2->getValue().getZExtValue(), val, 1, allocIdx);
                }
            }
          } else if (isa<llvm::AllocaInst>(ld->getPointerOperand())) {
            int allocIdx = getAllocInstrIdxUsedByStrct(
                cast<llvm::Instruction>(origGEP->getOperand(0)));
...
/*
 * this method is invoked when the argument --dump-file without --dump-bbs
 * It prints out the integer global variable and their corresponding values
 * after performing the symbolic execution
 */
void Dump::dumpState(std::ofstream &dumpFile, std::ofstream &primitiveLocals) {
  ...
  // This loop finds the corresponding value of each variable in the vector
  // dstate.globals, based on the matching address
  std::ofstream stringVarsGbls;
  stringVarsGbls.open("stringVarsGbls.txt");
  std::string stringGblStr;
  llvm::raw_string_ostream stringGblRst(stringGblStr);
    ...
    if (auto gvar = dyn_cast<llvm::GlobalVariable>(dglobal.global)) {
      if (gvar->hasName()) {
        ...
        if (it != dstate.memory.end()) {
          ...
          if (isa<llvm::PointerType>(cell.type)) {
            std::string wholeString;      
            auto st = addrToString.find(cell.value);
            if (st != addrToString.end()) {
              wholeString = st->second;
              stringGblRst << gvar->getName().str() << " " << wholeString
                           << "\n";
            }
          }
        }
  ...
  stringVarsGbls << stringGblRst.str();
  ...
  stringToVars.open("stringVarsLcl.txt");

  std::string primitiveStr, ptrToPrimitiveStr, stringVarsStr;

  llvm::raw_string_ostream primitiveVars(primitiveStr),
      ptrToPrimitiveVars(ptrToPrimitiveStr), stringVars(stringVarsStr);
  ...
  /*
   * Here I just tried to find if there is any elements in the dstate.stack that
   * might exist in the dstate.memory. This loop identifies the values of: 1-
   * primitive vars 2- struct vars 3- pointer to struct
   */
  ...
  for (auto &&ddstack : dstate.stack) {
    for (auto loc : ddstack.locals) {
      ...
      if (it != dstate.memory.end()) {
        ...
        if (isa<llvm::IntegerType>(cell.type)) {
          logger << std::get<0>(loc) << " --- " << std::get<1>(loc) << " --- "
                 << std::get<2>(loc) << " --- CellVal:: " << cell.value << "\n";
          if (auto *gep = dyn_cast<llvm::GetElementPtrInst>(std::get<3>(loc))) {
            // I need to update the implementation here for handling different
        ...
        for (auto &&ddstack : dstate.stack)
          for (auto loc : ddstack.locals) {
            if (std::get<2>(loc) == elem.first) {
              ...
              actualVal = cell.value;
              // here I need to find if the address has a corresponding string
              // in the string map addrToString
              auto st = addrToString.find(it->first);
              if (st != addrToString.end()) {
                strFlag = true;
                wholeString = st->second;
              }
              // here i need to find the index of the inst that is related to
              // the DerefAddr by searching the stack
              actualIdx = getActualIdx(ddstack.locals, it->first);
              loggerInst << "\tst: " << wholeString
                         << "---ActulIdx: " << actualIdx << "\n";
              if (ptrIdx != -1 &&
                  isa<llvm::AllocaInst>(std::get<3>(
                      loc))) { /*&& actualVal != -1 actualIdx != -1 &&*/
                if (!strFlag)
                  ptrToPrimitiveLocals << ptrIdx << " " << actualIdx << " "
                                       << actualVal << "\n";
                else {
                  stringVars << ptrIdx << " " << actualIdx << " " << wholeString
                             << "\n";
                }
              }
            }
